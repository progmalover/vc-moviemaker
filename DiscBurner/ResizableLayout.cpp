...
void CResizableLayout::ArrangeLayout()
{
	...
	// fix for dropdown combobox
	CList<HWND, HWND> listCB;
	...
	while (pos != NULL)
	{
		...
		if ((uFlags & (SWP_NOMOVE|SWP_NOSIZE)) != (SWP_NOMOVE|SWP_NOSIZE))
		{
			...
			if (layout.sWndClass == WC_COMBOBOX)
			{
				DWORD style = ::GetWindowLong(layout.hWnd, GWL_STYLE);
				if ((style & CBS_DROPDOWN) && ::SendMessage(layout.hWnd, CB_GETCURSEL, 0, 0L) >= 0)
				{
					HWND hWnd = ::GetFocus();
					if (hWnd != layout.hWnd && !::IsChild(layout.hWnd, hWnd))
					{
						::SendMessage(layout.hWnd, WM_SETREDRAW, FALSE, 0L);
						listCB.AddTail(layout.hWnd);
					}
				}
			}
		}
	...
	// fix for dropdown combobox
	HWND hWnd; 
	pos = listCB.GetHeadPosition();
	while (pos != NULL)
	{
		hWnd = listCB.GetNext(pos);
